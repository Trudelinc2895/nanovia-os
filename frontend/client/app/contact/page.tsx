...
import { PRO_PILOT } from "@/lib/pro-pilot";
...
function buildPilotMessage(form: OnboardingForm): string {
  return [
    `Demande Nanovia Pro Pilot — ${PRO_PILOT.priceCad} CAD / ${PRO_PILOT.deliveryWindow}`,
    "",
...
export default function ContactPage() {
  ...
  function openPilotMailto(currentForm: OnboardingForm) {
    window.location.href = `mailto:${PRO_PILOT.publicEmail}?subject=${encodeURIComponent(`Demande Nanovia Pro Pilot — ${PRO_PILOT.priceCad} CAD / ${PRO_PILOT.deliveryWindow}`)}&body=${encodeURIComponent(
      ...
      <div className="flex-1 pt-32 pb-24 px-6 max-w-3xl mx-auto w-full">
        <div className="text-center mb-12">
          ...
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            ...
            <a
              href={`mailto:${PRO_PILOT.publicEmail}`}
          ...
          <div className="mt-5 space-y-1 text-sm text-gray-400">
            <p>Pro Pilot {PRO_PILOT.deliveryWindow} — {PRO_PILOT.priceCad} $ CAD</p>
            <p>Suivi optionnel ensuite — {PRO_PILOT.optionalFollowUpCadMonthly} $ CAD / mois</p>
          </div>
        </div>
        ...
        {status === "ready" ? (
          <div className="bg-violet-900/20 border border-violet-500/30 text-violet-100 rounded-xl p-8 text-center">
            ...
            <p className="text-gray-400 mb-6">
              Si rien ne s&apos;ouvre, ecrivez directement a <span className="font-semibold text-white">{PRO_PILOT.publicEmail}</span>.
            </p>
